...
export default function InterviewApp() {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('http://localhost:8000/upload-resume', form);
      const res = await axios.get('http://localhost:8000/get-resume-info');
      setData(res.data);
      setStarted(false);
      setQuestion('');
      setAnswer('');
  ...
  const handleStartInterview = async () => {
    try {
      const res = await axios.post('http://localhost:8000/start-interview', {
        Name: data?.Name,
        Email: data?.Email,
        Phone: data?.Phone,
...
const handleSubmitAnswer = async () => {
  ...
  const endInterview = async () => {
    try {
      const res = await axios.get('http://localhost:8000/get-interview-feedback');
      setFinal(res.data);
    } catch (err) {
      console.error(err);
