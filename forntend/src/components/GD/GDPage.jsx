...
function GDPage() {
  ...
  const startGD = async () => {
    try {
      await axios.post("http://localhost:8000/start", { topic: selectedTopic });
...
const stopListeningAndSend = async () => {
  ...
  if (text) {
    const res = await axios.post("http://localhost:8000/speak", { text });
  ...
  const handleAiSpeak = async () => {
    try {
      const res = await axios.get("http://localhost:8000/ai_speak");
  ...
  const endGd = async () => {
    try {
      const res = await axios.get("http://localhost:8000/end");
